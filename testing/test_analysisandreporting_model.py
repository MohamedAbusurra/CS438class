...
def test_valid_report_creation(valid_filters, test_app):
...
def test_invalid_report_type_raises(bad_type, test_app):
    with test_app.app_context():
        with pytest.raises(ValueError, match="Invalid report type"):
            Report(project_id=1, reportType=bad_type)


def test_default_filters_when_none_provided(test_app):
    with test_app.app_context():
...
def test_filters_set_and_get_as_dict(valid_filters, test_app):
    with test_app.app_context():
...
def test_filters_broken_json_returns_empty(test_app, monkeypatch):
    with test_app.app_context():
...
def test_update_progress_valid(progress_value, test_app):
    with test_app.app_context():
...
def test_update_progress_invalid(bad_progress, test_app):
    with test_app.app_context():
        report = Report(project_id=1)
...
def test_mark_as_failed_sets_status_and_time(test_app):
    with test_app.app_context():
        report = Report(project_id=1)
...
def test_to_dict_serialization_fields(valid_filters, test_app):
    with test_app.app_context():
...
@patch("models.AnalysisandReporting.report.db")
...
@patch("models.AnalysisandReporting.report.Report.query")
...
def test_get_status_incomplete_report(test_app):
    with test_app.app_context():
        report = Report(project_id=1)
...
def test_get_status_completed_report(test_app):
    with test_app.app_context():
        report = Report(project_id=1)
